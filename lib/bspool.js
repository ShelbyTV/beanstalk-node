...
module.exports = {
  ...
  put : function(job, callback, tube, json_only){
    var self = this;
    self.respool.getResource(function(err, cl){
      cl.use(tube || self.putTube, function(err, res){
        if (json_only){
          job = JSON.stringify(job);
        } else {
          job = self.json_only ? JSON.stringify(job) :  encodeURIComponent(JSON.stringify(job));
        }
